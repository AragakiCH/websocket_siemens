...
export function Login() {
  ...
  const buscarProgramas = useCallback(
    async (appSel: string) => {
      ...
      setProgram('');
      try {
        const encontrados = await fetchRexrothPrograms(
          { ip, usuario: user, password },
          appSel
        );
        setPrograms(encontrados);
        setHint(`${encontrados.length} ${t('login.programsFound')}`);
        if (encontrados.length === 1) setProgram(encontrados[0]);
      } catch (e: any) {
