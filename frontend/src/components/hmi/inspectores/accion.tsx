...
/**
 * El panel «Valor con Unidad» de los widgets IMPORTADOS.
 *
 * POR QUÉ ESTÁ AQUÍ Y NO EN EL WIDGET «Valor con Unidad»
 * Son los mismos dos mandos, pero el widget de ese nombre es un TSX del
 * registry con su propio inspector; esto es para los ZIP, que no tienen
 * ninguno. Compartir el fichero es lo que hace que los dos escriban en el
 * MISMO sitio —`config.escritura` y `config.unidad`— en vez de inventarse
 * cada uno el suyo y discrepar el día que alguien cambie uno.
 *
 * POR QUÉ SE SEPARÓ DE «Acción»
 * Nació dentro del panel de Acción por una limitación que ya no existe: el
 * Inspector solo daba UN panel por widget, y «Acción» era el único que tenía
 * un ZIP. Quedaba raro —el modo de un widget no es una acción— y era confuso:
 * quien abría «Acción» buscando el botón se encontraba dos campos que no
 * tenían que ver. Ahora que el Inspector admite varios paneles, cada cosa
 * está donde se la busca.
 *
 * NO SE LE DA AL BOTÓN NI AL INTERRUPTOR, y eso es a propósito: `permiteEscritura()`
 * solo lo consultan los ZIP y el «Valor con Unidad». En un botón el Modo no
 * haría nada, y un mando que no hace nada es peor que no tenerlo.
 */
export function InspectorValorUnidad({ config, setConfig }: InspectorCtx) {
  // El modo NO es una acción: es en qué estado está el widget. Ver
  // `permiteEscritura` en `acciones.ts`.
