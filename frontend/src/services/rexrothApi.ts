...
/** Resultado de listar programas: incluye la app autodetectada. */
export interface RexrothProgramsResult {
  app: string;
  programas: string[];
}

/**
 * Lista los programas (POUs) de un ctrlX.
 * Si no se pasa `app`, el backend autodetecta la primera que tenga símbolos.
 */
export async function fetchRexrothPrograms(
  creds: RexrothCreds,
  app?: string
): Promise<RexrothProgramsResult> {
  const extra: Record<string, unknown> = {};
  if (app) extra.app = app;
  const r = await fetch('/rexroth/programs', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: toBody(creds, extra),
  });
  const data = await parse(r);
  return {
    app: data?.app ?? app ?? '',
